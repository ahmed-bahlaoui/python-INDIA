                ...
                if "summary" in doc and doc["summary"]:
                    ...
                    with tabs[0]:
                        if "analyse" in summary:
                            ...
                            with col1:
                                st.metric(
                                    "Type de document",
                                    analyse.get("type_document", "N/A"),
                                )
                            with col2:
                                st.metric(
                                    "Niveau de difficulté",
                                    analyse.get("niveau_difficulte", "N/A"),
                                )
                            with col3:
                                st.metric(
                                    "Temps de lecture",
                                    f"{analyse.get('temps_lecture_min', 0)} min",
                                )
                            ...
                            if "statistiques" in analyse:
                                ...
                                with col1:
                                    st.metric(
                                        "Pages estimées",
                                        stats.get("nb_pages_estime", 0),
                                    )
                                with col2:
                                    st.metric(
                                        "Nombre de mots", f"{stats.get('nb_mots', 0):,}"
                                    )
                                with col3:
                                    st.metric(
                                        "Concepts uniques",
                                        stats.get("nb_concepts_uniques", 0),
                                    )
                    ...
                    with tabs[2]:
                        if "mindmap" in summary:
                            ...
                            if mindmap.get("concepts_principaux"):
                                ...
                                cols = st.columns(
                                    min(3, len(mindmap["concepts_principaux"]))
                                )
                                for i, concept in enumerate(
                                    mindmap["concepts_principaux"]
                                ):
                                    with cols[i % 3]:
                                        st.markdown(
                    ...
                    with tabs[3]:
                        if summary.get("timeline"):
                            ...
                            st.info(
                                "Aucune timeline disponible (document non chronologique)"
                            )
                        ...
                        if summary.get("flashcards"):
                            ...
                            for i, card in enumerate(summary["flashcards"]):
                                with st.container():
                                    ...
                                    st.markdown(
                                        f"**Carte {i+1}** {difficulty_badge} {card.get('difficulte', 'Moyen')}"
                                    )

                                    # Question dans un expander
                                    with st.expander(f"❓ {card.get('question', '')}"):
                                        st.success(
                                            f"✅ **Réponse:** {card.get('reponse', '')}"
                                        )
                                    st.divider()
                        else:
                    ...
                    st.info(
                        "Aucun résumé disponible. Cliquez sur 'Analyser tous les documents'."
                    )
